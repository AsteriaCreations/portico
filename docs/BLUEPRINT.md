...
**One-time payment methods**: `payment_methods.one_time_only` — a method flagged true (Venmo and PayPal, seeded; plus a neutral `electronic` code) may be used once per member, ever, but **only for entry-tier payments** — check-in itself and a per-event add-on day pass; using any one appends a dated note to `hospitality_note` and disables *all* one-time methods for that member thereafter, for entry (`Member::hasUsedOneTimeMethod()`, computed from attendance + day-pass history, never stored). It is deliberately **not** a gate on a subscription/membership purchase — a member may pay their membership by Venmo/PayPal/electronic any number of times, standalone (`CheckIn::purchaseSubscriptionAction()`) or via the Subscriptions resource's bulk purchase; "one-time" is a desk/entry rule, not a membership rule. Replaces the old hardcoded "Venmo is one use" rule.

**Payment method transaction fee**: `payment_methods.transaction_fee` — a flat per-transaction surcharge (Venmo/PayPal/electronic seeded at $2) modelling the real processor cost of accepting that method. Folded directly into the recorded `amount_paid` (attendance, subscription, or day-pass row) whenever that method is selected and the underlying total being charged is greater than $0 — never tracked as a separate ledger, so `RegisterShiftService`'s reconciliation needs no changes. Applied everywhere a payment method is selectable: check-in (`CheckIn::checkInAction()`), a standalone subscription purchase (`CheckIn::purchaseSubscriptionAction()` and `ListSubscriptions::bulkPurchaseAction()` — folded onto the first covered month's row for a multi-month bundle), and an add-on day pass (`CheckIn::purchaseAddOnDayPassAction()`). Not applied to a miscellaneous payment (`recordMiscPaymentAction()`), whose `amount` is a free-typed figure with no separate "price" to add a fee onto. `PaymentMethod::feeFor($code)`/`feeHelperText($code)` are the two entry points — the latter renders live next to a `payment_method` Select so staff see the fee before it's charged.
