...
- Guest registration (sponsor-gated, full identity capture); can be switched off, optionally allowed during probation, and optionally capped per member per night
...
- Rank rule for staff accounts — nobody edits, resets, deletes or grants a role above their own (only an Owner manages Owners; while there's no active Owner, an Admin may grant Owner)
...
- Feature Flags page (vouchers, add-ons, showrunner comp requests, manager perk, suspensions, pool, prepay, register shifts, showrunner payouts, instructor payouts, visit notes, behavior notes, guests); turning one on sends setup reminders to the notification bell, linking to each screen still needing setup
